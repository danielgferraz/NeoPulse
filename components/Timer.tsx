...
const Timer: React.FC<TimerProps> = ({ timeLeft, isActive, duration, onToggle, onReset, onAdjust }) => {
  ...
  useEffect(() => {
    if (isActive) {
      if (timeLeft <= 4 && timeLeft > 0 && timeLeft !== lastBeepedTime.current) {
        playBeep(440, 0.05);
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-sm mx-auto select-none">
      <div className="relative flex items-center justify-center mb-8" style={{ width: size, height: size }}>
        <div className={`absolute inset-0 rounded-full blur-3xl transition-all duration-1000 opacity-20 ${
          isActive ? (isUrgent ? 'bg-red-500 scale-110' : 'bg-[#00FF41] scale-105') : 'bg-transparent'
        }`} />
        ...
        <svg viewBox={`0 0 ${size} ${size}`} className="absolute w-full h-full -rotate-90">
          ...
          <circle
            ...
            className={`transition-all duration-1000 ease-linear ${
              isUrgent ? 'text-red-500' : 'text-[#00FF41]'
            }`}
          />
        </svg>
      ...
      <div className="flex gap-3 w-full max-w-[272px] mb-8">
        ...
        <button
          onClick={onToggle}
          className={`flex-1 h-12 rounded-2xl flex items-center justify-center text-black font-black text-xs tracking-widest transition-all shadow-xl active:scale-95 ${
            isActive ? 'bg-zinc-200' : 'bg-[#00FF41]'
          }`}
        >
          {isActive ? (
